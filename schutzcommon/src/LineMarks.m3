      ...
      ; PROCEDURE GnlTeTfsModTok ( EstRef : EstHs . EstRefTyp ) 
        ...
        ; BEGIN 
            ...
            => IF StartMark . TkmEstNodeNo 
                  = EstAbsNodeNo + GnlTeEstTravInfo . EtiChildRelNodeNo 
                  AND StartMark . TkmKind = MarkKindTyp . Plain 
        ...
        ; BEGIN (* GnlTeTfsInsTok *) 
          ...
          ; CASE GnlState 
            ...
            => (* The actual start point must be in a trailing mod. *) 
               Assert 
                 ( StartMark . TkmKind IN Marks . MarkKindSetEstLeaf 
...
(* TODO: ^Decide on this. *) 
      ...
      ; PROCEDURE GnlTeTfsLineBreak ( FsKind : FsKindTyp )
        ...
        ; BEGIN (* GnlTeTfsLineBreak *) 
          ...
          ; CASE GnlState 
            OF GnlStateTyp . GnlStateStartAtBeg 
            => IF StartMark . TkmKind IN Marks . MarkKindSetEstLeaf 
      ...
      ; BEGIN (* GnlTeTraverseFs *) 
        ...
        ; BEGIN (* Block  GnlTeTraverseFs *) 
            ...
            => IF GnlState IN GnlStateSetStart 
                  AND StartMark . TkmKind IN Marks . MarkKindSetEstLeaf 
        ...
        ; IF EstRef # NIL 
          ...
          ; CASE GnlState 
            ...
            => (* Choose the Est child to descend to. *) 
               ...
               ; IF StartMark . TkmKind = MarkKindTyp . RightSibFmtNo 
            ...
            ; CASE StartMark . TkmKind 
              OF MarkKindTyp . ChildFmtNo 
              => IF EstAbsNodeNo = StartMark . TkmEstNodeNo 
                 THEN GnlTeStartFmtNo := StartMark . TkmFmtNo 
                 END (* IF *)
...
*) 
    ...
    ; IF GnlNextLineErr > 0 
      ...
      END (* IF *) 
    ; Assert
        ( NewMark . TkmKind # MarkKindTyp . Null 
...
(* FIXME: I don't think so.  Bl is viewed as zero-width by the containing text. *)
      ...
      ; PROCEDURE GplTeTfsStringModAtEndingMark 
        ...
        = BEGIN (* GplTeTfsStringModAtEndingMark *) 
            RETURN 
              ExistingMark . TkmKind = MarkKindTyp . Plain 
              AND EstAbsNodeNo + GplTeEstTravInfo . EtiChildRelNodeNo 
                  = ExistingMark . TkmEstNodeNo 
              AND StartAtEnd = ExistingMark . TkmStartAtEnd 
...
(* CHECK: ^Wasn't this requirement obviated when we went to node numbers 
      ...
      ; PROCEDURE GplTeTfsIsAtEndingFmtNoMark ( ) : BOOLEAN 
        ...
        = BEGIN (* GplTeTfsIsAtEndingFmtNoMark *) 
            CASE ExistingMark . TkmKind 
            OF MarkKindTyp . ChildFmtNo 
            => RETURN 
                 EstAbsNodeNo = ExistingMark . TkmEstNodeNo 
                 AND FsNodeRef . FsFmtNo = ExistingMark . TkmFmtNo 
            | MarkKindTyp . LeftSibFmtNo 
            => RETURN 
            ...
            ELSE (* Make it LeftSibFmtNo *) 
              NewMark 
                := Marks . TokMarkTyp 
                     ...
                     , TkmEstRef := GplTeRightwardChildRef 
                     , TkmKind := MarkKindTyp . LeftSibFmtNo 
                     , TkmFmtNo := FsNodeRef . FsFmtNo 
                     , TkmStartAtEnd := FALSE 
                     , TkmIsImpliedNewLine := FALSE 
...
(* CHECK: This shouldn't happen either. *) 
      ...
      ; BEGIN (* GplTeTraverseFs *) 
        ...
        ; BEGIN (* Block  GplTeTraverseFs *) 
            ...
            ; Assert 
                ( ExistingMark . TkmKind = MarkKindTyp . Null 
                  OR GplTeTfsIsAtEndingFmtNoMark ( ) 
      ...
      ; IF EstRef # NIL 
        ...
        ; CASE GplState 
          ...
          ; CASE StartMark . TkmKind 
            OF MarkKindTyp . ChildFmtNo 
            => IF EstAbsNodeNo = StartMark . TkmEstNodeNo 
               THEN GplTeStartFmtNo := StartMark . TkmFmtNo 
               END (* IF *) 
            ...
            => IF GplTeEstTravInfo . EtiChildNo + 1 
                  ...
                  AND EstAbsNodeNo + GplTeRightwardChildRelNodeNo 
                      = StartMark . TkmEstNodeNo 
               THEN 
                 GplTeStartFmtNo := StartMark . TkmFmtNo 
               END (* IF *) 
        ...
        ELSE 
          NewMark 
            := Marks . TokMarkTyp 
                 ...
                 , TkmEstRef := LEstTravInfo . EtiChildLeafElem . LeChildRef 
                 , TkmKind := MarkKindTyp . LeftSibFmtNo 
...
(* TODO: Do this in a better way.  Generalize GetNextLine so it can be
         ...
         requiring us to back up with GetPrevLine.
*) 
    ; GetNextLine 
        ( Lang  
        , RootEstRef  
