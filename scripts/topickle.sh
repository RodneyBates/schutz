...
# File topickle.sh, subdirectory scripts, Schutz Semantic editor. 

# Set up to use Pickle (not Pickle2).
...
do 
  cp -p $File $File.~bakpickle~
  sed -e 's/; IMPORT Pickle2 AS Pickle (\* The Pickle2 mess. \*)/; IMPORT Pickle (* The Pickle2 mess. *)/' $File > $File.temp
  ...
  then 
    ...
    mv $File.temp $File 
    echo "$File changed to use Pickle."
  fi 
done
